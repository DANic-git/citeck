...
if [[ $STATUS != 0 ]]; then
	...
	tar cfz /backup/weekly/files/$(date +"%y-%m-%d_%H-%M-%S").tar.gz --exclude='postgresql/*' ./alf_data | adddate
	if [[ $? != 0 ]]; then
	...
	log "Выгрузка базы"
	/opt/alfresco-4.2.f/postgresql/bin/pg_dump -w -U alfresco alfresco> /backup/weekly/sql/$(date +"%y-%m-%d_%H-%M-%S").sql | adddate
	if [[ $? != 0 ]]; then
	...
	rsync -zvr /backup/weekly $BS::$BP | adddate
	if [[ $? != 0 ]]; then
	...
	cd /backup/weekly/files/
	if [[ $? != 0 ]]; then
		log "Нет каталога /backup/weekly/files/"
		exit
	fi
	rm -f `ls -t --full-time | awk '{if (NR > 2)printf("%s ",$9);}'`
	cd /backup/weekly/sql/
	if [[ $? != 0 ]]; then
		log "Нет каталога /backup/weekly/files/"
		exit
	fi	
